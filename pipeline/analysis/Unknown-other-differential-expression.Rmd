...
plot <- box %>% 
  mutate(origin = ifelse(origin == "Unknown & Other vs Assigned", "Unknown & Other vs all Assigned", origin)) %>% 
  mutate(origin = ifelse(origin == "Unknown & Other vs Stromal", "Unknown & Other vs Stromal", origin)) %>% 
...
plot <- plot + geom_boxplot(outlier.colour = "lightgrey") +
    guides(color = FALSE) +
...
pdf(paste0(params$output_dir, "Unknown-other-differential-expression-", params$cohort, ".pdf"), width = 7, height = 8)
  plot
dev.off()
```
