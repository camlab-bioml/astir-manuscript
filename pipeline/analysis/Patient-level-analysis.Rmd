...
plotClusteringHeatmapCore <- function(freqPerCore, cl = NA){
  if(params$cohort == "basel"){
    col_fun <- colorRamp2(c(min(freqPerPatient$OSmonth),
                          max(freqPerPatient$OSmonth)/2,
                          max(freqPerPatient$OSmonth)), viridis(3))
  ...
  if(params$cohort == "basel"){
    cluster_annotation <- as.factor(cutree(col_dend,k=10))
  ...
  hm <- Heatmap(t(as.matrix(freqPerCore %>% 
                              select(`B cells`:Unknown))),
                column_dend_height = unit(5, "cm"),
  ...
  list(hm = hm, dend = col_dend)
}

plotClusteringHeatmapCore(freqPerCore)$hm

freqPerCoreNorm <- freqPerCore %>% 
  mutate(across(`B cells`:Unknown, function(x){scale(x, center = TRUE, scale = TRUE)}))

coreNormPlot <- plotClusteringHeatmapCore(freqPerCoreNorm)
coreNormPlot$hm
```

```{r fig.height=12, fig.width=5, eval=params$cohort == "basel"}
metadata <- metadata %>% 
  dplyr::rename(perc_tumorcells = `%tumorcells`, 
                perc_normalepithelialcells = `%normalepithelialcells`,
                perc_stroma = `%stroma`, perc_inflammatory = `%inflammatorycells`)

# removed tumor size, %inflamatory because these are numeric
core_cluster_chr <- freqPerCoreNorm %>% 
  mutate(cluster = cutree(coreNormPlot$dend,k=10)) %>% 
  select(core, cluster) %>% 
  left_join(select(metadata, core, grade, diseasestatus, 
                   ERStatus, PRStatus, HER2Status, clinical_type, response), by = "core") %>% 
  column_to_rownames("core") %>% 
  mutate(grade = as.factor(grade)) %>% 
  pivot_longer(cols = -cluster, values_to = "value", names_to = "covariate")
...
core_cluster_chr %>% 
  ...
  labs(x = "Cluster", y = "Number of cores") +
  facet_wrap(~covariate, ncol = 1, scale = "free") +
  astir_paper_theme()
```



Test for statistical significance by doing a PCA regression
```{r eval= params$cohort == "basel"}
core_freq_pca <- freqPerCoreNorm %>% 
  column_to_rownames("core")

core.pca <- prcomp(core_freq_pca, scale. = T, center = T)

fviz_eig(core.pca)
fviz_pca_ind(core.pca, repel = TRUE)
...
covariates_summary <- core.pca$x %>% 
  as.data.frame() %>% 
  rownames_to_column("core") %>% 
  pivot_longer(-core, values_to = "loadings", names_to = "PCs") %>% 
  left_join(select(metadata, core, grade, diseasestatus, 
                   ERStatus, PRStatus, HER2Status, clinical_type, response), by = "core") %>% 
  mutate(grade = as.character(grade)) %>% 
  pivot_longer(cols = c(grade, diseasestatus, ERStatus, PRStatus, HER2Status, clinical_type, response), 
               names_to = "covariate", values_to = "category")

covariates_summary %>% 
  filter(PCs %in% c("PC1", "PC2", "PC3")) %>% 
  group_by(PCs, covariate) %>% 
  do(tidy(lm(loadings ~ category, data = .))) %>%
  datatable()
```

```{r fig.height=4, fig.width=12, eval=params$cohort == "basel"}
core_cluster_int <- freqPerCoreNorm %>% 
  mutate(cluster = cutree(coreNormPlot$dend,k=11)) %>% 
  select(core, cluster) %>% 
  left_join(select(metadata, core, tumor_size, perc_tumorcells, perc_normalepithelialcells, 
                   perc_stroma, perc_inflammatory, DFSmonth, OSmonth), by = "core") %>% 
  column_to_rownames("core") %>% 
  pivot_longer(cols = -cluster, values_to = "value", names_to = "covariate")
...
core_cluster_int %>% 
  ...
  geom_boxplot() +
  facet_wrap(~covariate, scales = "free_y", ncol = 7) +
  astir_paper_theme()
```

Anova on the above results
```{r eval= params$cohort == "basel"}
anovas <- core_cluster_int %>% 
  group_by(covariate) %>% 
  do(Model = aov(value ~ cluster, data = .))

lapply(anovas$Model, function(x){broom::tidy(x)}) %>% 
  bind_rows() %>% 
  filter(term == "cluster") %>% 
  mutate(pathway = sort(anovas$covariate)) %>% 
  column_to_rownames("pathway")

# Will follow up with tukeys test if any of them are significant
```


```{r fig.width = 11, eval = params$cohort == "basel"}
patient_cores <- freqPerCoreNorm %>% 
  mutate(cluster = as.factor(cutree(coreNormPlot$dend,k=10))) %>% 
  select(core, cluster) %>% 
  left_join(select(metadata, core, PID), by = "core")

patient_cores_summary <- patient_cores %>% 
  group_by(PID, cluster) %>% 
  tally() %>% 
  mutate(PID = as.factor(PID))

patient_cores_summary

patient_cores_summary %>% 
  ggplot(aes(x = PID, y = n, fill = cluster)) +
