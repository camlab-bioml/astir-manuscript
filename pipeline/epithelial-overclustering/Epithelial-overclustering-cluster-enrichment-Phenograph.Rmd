...
```{r}
cell_subset <- read_csv("C:/Users/micha/CampbellLab/imc-2020/output/squirrel/results/epithelial_overclustering/luminal-0.3.csv")

sce <- readRDS(params$cells)[,cell_subset$cell_id]

percent_luminal <- 0.3#params$percent_luminal
```
...
imc.pheno.cluster <- cytof_cluster(xdata = imc.data, method = "Rphenograph",
                                   Rphenograph_k = params$cluster_options)
...
write_csv(assigned_cells, paste0(params$output_results,
                                 ...
                                 params$markers, "-", params$cluster_options, "-", 
                                 percent_luminal, ".csv"))
```