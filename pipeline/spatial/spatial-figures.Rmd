...
params:
  basel_output: ../../output/v6/spatial/rds_output_basel.rds
  zurich1_output: ../../output/v6/spatial/rds_output_zurich1.rds
  basel_metadata: mdata.csv
  kaplan_meier: km.pdf
  pathway_fig: pw.pdf
...
```{r}
pdf(params$spatial_heatmap, width = 8, height = 4.5)
draw(ht, merge_legend = TRUE, heatmap_legend_side = "bottom", annotation_legend_side = "bottom",
...
ggplot(df_test, aes(x = -log10(p.value), y = fct_reorder(state, -log10(p.value)))) +
  geom_point(aes(fill = estimate), shape = 21, size = 6) +
...
```{r}
pdf(params$pathway_fig, width = 8.2, height = 4)
print(last_plot())
dev.off()
```
